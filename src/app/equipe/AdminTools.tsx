...
export default function AdminTools() {
  ...
  async function saveManual() {
    ...
    try{
      ...
      setManualMode(false);
      setMessage("Cadastro pronto. Agora clique em Atender aluno agora.");
    }catch(e){setMessage(e instanceof Error?e.message:"Falha");}
    finally{setLoading(false);}
  }
  ...
  async function issue(contractId: string) {
    ...
    try {
      // O token continua existindo internamente por segurança, mas não é mostrado ao operador nem ao aluno.
      ...
      const sessionResponse = await fetch("/api/public/session", {
        ...
        body: JSON.stringify({ accessId: data.accessId })
      });
