...
function friendlyStatus(status:string){
  const labels:Record<string,string>={
    AWAITING_SIGNATURE:"Aguardando assinatura",SIGNED_RECEIVED:"Termo recebido",FEE_PENDING:"Aguardando pagamento da taxa",READY_TO_CANCEL:"Aguardando validação final",
    UNDER_REVIEW:"Em conferência",APPROVED:"Aprovado",MANUAL_REVIEW:"Validação manual necessária",EVO_CANCEL_REQUESTED:"Cancelamento enviado",
    EVO_CANCELLED:"Contrato cancelado",CANCELLED_CONFIRMED:"Contrato cancelado",REFUND_PENDING:"Estorno pendente",REFUND_REGISTERED:"Estorno registrado",
...
export default function ClientWizard(){
  ...
  const [address,setAddress]=useState(""),[rg,setRg]=useState(""),[pixKey,setPixKey]=useState(""),[accepted,setAccepted]=useState(false),[privacyAccepted,setPrivacyAccepted]=useState(false);
  ...
  const [loading,setLoading]=useState(true),[error,setError]=useState(""),[noSession,setNoSession]=useState(false),[result,setResult]=useState<{protocol:string;status:string;message:string;feePayment?:{amount:number;pixKey:string;pixName:string}}|null>(null);
    ...
    (async()=>{
      try{
        ...
        if(latest&&["AWAITING_SIGNATURE","SIGNED_RECEIVED"].includes(latest.status)){
          ...
          setResult({protocol:latest.protocol,status:latest.status,message:"Sua solicitação já foi registrada e está em andamento."});
        }
  ...
  async function generateTerm(){setLoading(true);setError("");try{const d=await req("/api/public/cancel",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({reasonCode:reasonMap[reason],reasonDetails:reason==="Outro"?reasonDetails:undefined,desiredDate,termVersion:"EV-CAN-2026.09-v7",accepted:true,privacyAccepted:true,requesterAddress:address,requesterRg:rg,pixKey:contract?.recurring?undefined:pixKey})});setProtocol(d.protocol);setTermUrl(d.termUrl);setStep(3)}catch(e){setError(e instanceof Error?e.message:"Não foi possível gerar o termo.")}finally{setLoading(false)}}
  ...
  if(noSession)return <div className="wizard-card glass access-help"><p className="eyebrow">CANCELAMENTO ONLINE</p><h1>Sua sessão terminou</h1><p>Volte ao início e confirme novamente seu nome completo, data de nascimento, e-mail e o código de segurança.</p><a className="btn primary" href="/">Voltar ao início</a></div>;
  if(result)return <div className="wizard-card glass"><div className="wizard-head"><div><p className="eyebrow">SOLICITAÇÃO REGISTRADA</p><h1>Pedido recebido</h1></div></div><div className="wizard-body"><div className="protocol-preview success-panel"><span>Protocolo</span><b>#{result.protocol}</b><span>{friendlyStatus(result.status)}</span></div><div className="info-box">{result.message}</div>{result.status==="FEE_PENDING"&&result.feePayment&&<div className="fee-payment-box"><h3>Pagamento da taxa</h3><div><span>Valor</span><b>{money.format(result.feePayment.amount)}</b></div>{result.feePayment.pixKey?<><div><span>PIX</span><b>{result.feePayment.pixKey}</b></div><small>Favorecido: {result.feePayment.pixName}</small></>:<small>A equipe informará a forma de pagamento cadastrada para a taxa.</small>}<label>Enviar comprovante<input type="file" accept="application/pdf,image/jpeg,image/png" onChange={e=>setFeeReceipt(e.target.files?.[0]||null)}/></label>{feeReceipt&&<button className="btn primary" disabled={loading} onClick={uploadFeeReceipt}>{loading?"Enviando...":"Enviar comprovante"}</button>}{feeReceiptMessage&&<div className="info-box">{feeReceiptMessage}</div>}</div>}<div className="info-box">Nenhum contrato é cancelado automaticamente. A Evolution fará a conferência final de pendências, dados e termo antes da conclusão.</div></div><div className="wizard-actions"><a className="btn primary" href="/">Concluir</a></div></div>;

  return <div className="wizard-card glass">
    <div className="wizard-head"><div><p className="eyebrow">CANCELAMENTO ONLINE</p><h1>Cancelar contrato</h1></div><span className="simple-progress-label">{steps[step]}</span></div>
    ...
    <div className="wizard-body">{error&&<div className="error-box">{error}</div>}
      {step===0&&contract&&<><div className="welcome-line">Olá, <b>{customerName.split(" ")[0]}</b>. Confira os dados do contrato.</div><div className="contract-card selected"><span>CONTRATO ATIVO</span><h3>{contract.planName}</h3><p><b>Unidade:</b> {contract.unit}</p><p><b>Início:</b> {new Date(contract.startDate).toLocaleDateString("pt-BR")}</p><p><b>Tipo:</b> {contract.recurring?"Anual recorrente":"Anual"}</p>{contract.amountPaid>0&&<strong>Valor do plano: {money.format(contract.amountPaid)}</strong>}</div></>}
      {step===1&&<><label>Motivo do cancelamento</label><div className="choice-grid">{Object.keys(reasonMap).map(r=><button type="button" className={reason===r?"selected":""} onClick={()=>setReason(r)} key={r}>{r}</button>)}</div>{reason==="Outro"&&<label>Descreva o motivo<textarea value={reasonDetails} maxLength={1000} onChange={e=>setReasonDetails(e.target.value)}/></label>}<label>Data pretendida para o cancelamento<input type="date" value={desiredDate} onChange={e=>setDesiredDate(e.target.value)}/></label></>}
      {step===2&&<>{preview?.kind==="ANNUAL"&&preview.eligible&&preview.calculation?<div className="calc-panel"><p className="eyebrow">CÁLCULO DO PLANO ANUAL</p><div><span>Valor total do plano</span><b>{money.format(preview.calculation.totalContractValue)}</b></div><div><span>Valor mensal de referência</span><b>{money.format(preview.calculation.monthlyReference)}</b></div><div><span>Meses utilizados</span><b>{preview.calculation.monthsUsed}</b></div><div><span>Meses restantes</span><b>{preview.calculation.monthsRemaining}</b></div><div><span>Saldo dos meses restantes</span><b>{money.format(preview.calculation.unusedBalance)}</b></div><div><span>14,4% sobre o valor total</span><b>- {money.format(preview.calculation.advanceDeduction)}</b></div><div><span>10% sobre o valor total</span><b>- {money.format(preview.calculation.contractFee)}</b></div><hr/><div className="total"><span>Estorno previsto</span><b>{money.format(preview.calculation.estimatedRefund)}</b></div><small>Cálculo automático em prévia. O Ruy/equipe confere pendências e dados antes da autorização final.</small></div>:preview?.kind==="RECURRING"?<div className="calc-panel"><p className="eyebrow">PLANO ANUAL RECORRENTE</p><div><span>Estorno</span><b>Não se aplica</b></div><div><span>Multa prevista no termo</span><b>{money.format(preview.feeAmount||0)}</b></div><div><span>Antecedência prevista</span><b>30 dias da próxima mensalidade</b></div><hr/><p>{preview.reason}</p></div>:<div className="info-box">{preview?.reason||"Os valores serão conferidos antes da conclusão."}</div>}</>}
      {step===3&&<>{!protocol?<><div className="term-box"><h3>Gerar termo oficial</h3><p>Preencha os dados necessários para reproduzir o termo padrão da Evolution. Os campos sensíveis são armazenados cifrados.</p></div><label>RG<input value={rg} maxLength={40} onChange={e=>setRg(e.target.value)} placeholder="Número do RG"/></label><label>Endereço completo<input value={address} maxLength={240} onChange={e=>setAddress(e.target.value)} placeholder="Rua, número, bairro, cidade"/></label>{contract&&!contract.recurring&&(preview?.calculation?.estimatedRefund||0)>0&&<label>Chave PIX para receber o estorno<input value={pixKey} maxLength={180} onChange={e=>setPixKey(e.target.value)} placeholder="Informe a chave para recebimento"/></label>}<label className="check"><input type="checkbox" checked={privacyAccepted} onChange={e=>setPrivacyAccepted(e.target.checked)}/><span>Declaro ciência do tratamento dos meus dados pessoais para identificação e processamento desta solicitação de cancelamento, incluindo consulta ao cadastro vinculado ao meu contrato.</span></label><label className="check"><input type="checkbox" checked={accepted} onChange={e=>setAccepted(e.target.checked)}/><span>Li as informações do plano e confirmo que desejo solicitar o cancelamento.</span></label></>:<div className="download-panel"><h3>Termo pronto</h3><b>Protocolo #{protocol}</b><a className="btn primary" href={termUrl}>Baixar termo em PDF</a><span>Assine o documento e envie o arquivo assinado na próxima etapa.</span></div>}</>}
      {step===4&&<>{alreadySigned?<div className="upload-zone"><p className="eyebrow">DOCUMENTO RECEBIDO</p><h3>Termo assinado recebido</h3><p>Finalize para enviar o pedido à conferência da Evolution.</p></div>:<div className="upload-zone"><p className="eyebrow">ENVIAR TERMO ASSINADO</p><h3>Selecione o arquivo</h3><p>PDF, JPG ou PNG • até 8 MB</p><input type="file" accept="application/pdf,image/jpeg,image/png" onChange={e=>setSignedFile(e.target.files?.[0]||null)}/>{signedFile&&<b>{signedFile.name}</b>}</div>}</>}
    </div>
    <div className="wizard-actions"><button className="btn secondary" disabled={step===0||loading} onClick={()=>setStep(Math.max(0,step-1))}>Voltar</button>
      ...
      {step===1&&<button className="btn primary" disabled={!desiredDate||loading} onClick={loadPreview}>{loading?"Calculando...":"Ver valores"}</button>}
      {step===2&&<button className="btn primary" onClick={()=>setStep(3)}>Continuar</button>}
      {step===3&&!protocol&&<button className="btn primary" disabled={!accepted||!privacyAccepted||!address.trim()||!rg.trim()||loading} onClick={generateTerm}>{loading?"Gerando...":"Gerar termo"}</button>}
      {step===3&&protocol&&<button className="btn primary" onClick={()=>setStep(4)}>Enviar termo assinado</button>}
      {step===4&&!alreadySigned&&signedFile&&<button className="btn primary" disabled={loading} onClick={uploadAndFinalize}>{loading?"Enviando...":"Enviar para validação"}</button>}{step===4&&alreadySigned&&protocol&&<button className="btn primary" disabled={loading} onClick={finalizeExisting}>{loading?"Enviando...":"Enviar para validação"}</button>}
    </div>
  </div>;
}
