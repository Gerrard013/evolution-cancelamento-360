...
function friendlyStatus(status:string){
  const labels:Record<string,string>={
    ...
    UNDER_REVIEW:"Em conferência",APPROVED:"Aprovado",MANUAL_REVIEW:"Em conferência",EVO_CANCEL_REQUESTED:"Cancelamento enviado",
...
export default function ClientWizard(){
  ...
  const [address,setAddress]=useState(""),[pixKey,setPixKey]=useState(""),[accepted,setAccepted]=useState(false);
  ...
  async function generateTerm(){setLoading(true);setError("");try{const d=await req("/api/public/cancel",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({reasonCode:reasonMap[reason],reasonDetails:reason==="Outro"?reasonDetails:undefined,desiredDate,termVersion:"EV-CAN-2026.09-v6",accepted:true,requesterAddress:address,contactEmail:"",pixKey:contract?.recurring?undefined:pixKey})});setProtocol(d.protocol);setTermUrl(d.termUrl);setStep(3)}catch(e){setError(e instanceof Error?e.message:"Não foi possível gerar o termo.")}finally{setLoading(false)}}
  ...
  if(noSession)return <div className="wizard-card glass access-help"><p className="eyebrow">CANCELAMENTO ONLINE</p><h1>Sua sessão terminou</h1><p>Volte ao início e informe novamente sua matrícula e data de nascimento.</p><a className="btn primary" href="/">Voltar ao início</a></div>;
  ...
  return <div className="wizard-card glass">
    ...
    <div className="wizard-body">{error&&<div className="error-box">{error}</div>}
      {step===0&&contract&&<><div className="welcome-line">Olá, <b>{customerName.split(" ")[0]}</b>. Confira os dados do contrato.</div><div className="contract-card selected"><span>CONTRATO ATIVO</span><h3>{contract.planName}</h3><p><b>Unidade:</b> {contract.unit}</p><p><b>Início:</b> {new Date(contract.startDate).toLocaleDateString("pt-BR")}</p><p><b>Tipo:</b> {contract.recurring?"Recorrente":"Anual"}</p>{contract.amountPaid>0&&<strong>Valor do plano: {money.format(contract.amountPaid)}</strong>}</div></>}
    ...
    <div className="wizard-actions"><button className="btn secondary" disabled={step===0||loading} onClick={()=>setStep(Math.max(0,step-1))}>Voltar</button>
      ...
      {step===3&&!protocol&&<button className="btn primary" disabled={!accepted||!address.trim()||loading} onClick={generateTerm}>{loading?"Gerando...":"Gerar termo"}</button>}
