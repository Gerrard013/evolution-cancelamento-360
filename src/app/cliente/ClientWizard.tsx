...
type Preview = { eligible:boolean; reason?:string; feeReference?:number; noticeDays?:number; rule?:{version:string;name:string;percentage:number}; calculation?:{unusedBalance:number;deduction:number;estimatedRefund:number;advanceDeduction?:number;contractFee?:number} };
...
export default function ClientWizard(){
  ...
  async function generateTerm(){setLoading(true);setError("");try{const d=await req("/api/public/cancel",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({reasonCode:reasonMap[reason],reasonDetails:reason==="Outro"?reasonDetails:undefined,desiredDate,termVersion:"EV-CAN-2026.09-v5",accepted:true,requesterAddress:address,contactEmail:"",pixKey:contract?.recurring?undefined:pixKey})});setProtocol(d.protocol);setTermUrl(d.termUrl);setStep(3)}catch(e){setError(e instanceof Error?e.message:"Falha ao gerar termo")}finally{setLoading(false)}}
  ...
  if(noSession)return <div className="wizard-card glass access-help"><p className="eyebrow">ATENDIMENTO DO ALUNO</p><h1>O atendimento ainda não foi iniciado</h1><p>Por segurança, o aluno não consulta contratos apenas digitando uma matrícula previsível. A equipe localiza o cadastro no painel e abre este formulário no próprio atendimento.</p><div className="help-steps"><div><b>1. Informe sua matrícula EVO à equipe</b><span>Exemplo: 29965.</span></div><div><b>2. A equipe confere o contrato</b><span>Condor ou Umarizal, plano e situação ativa.</span></div><div><b>3. A equipe clica em “Atender aluno agora”</b><span>Esta tela abre automaticamente, sem link e sem código para copiar.</span></div></div><a className="btn primary" href="/equipe">Ir para a área da equipe</a></div>;

  if(result)return <div className="wizard-card glass"><div className="wizard-head"><div><p className="eyebrow">PEDIDO REGISTRADO</p><h1>Solicitação enviada</h1></div></div><div className="wizard-body"><div className="protocol-preview success-panel"><span>Protocolo</span><b>#{result.protocol}</b><span>{friendlyStatus(result.status)}</span></div><div className="info-box">{result.message} Guarde o protocolo para acompanhamento.</div></div><div className="wizard-actions"><a className="btn primary" href="/equipe">Voltar para a central da equipe</a></div></div>;
  ...
  return <div className="wizard-card glass">
    ...
    <div className="wizard-body">{error&&<div className="error-box">{error}</div>}
      ...
      {step===2&&<>{preview?.eligible&&preview.calculation?<div className="calc-panel"><p className="eyebrow">PRÉVIA DE ESTORNO — PLANO ANUAL</p><div><span>Saldo proporcional não utilizado</span><b>{money.format(preview.calculation.unusedBalance)}</b></div>{typeof preview.calculation.advanceDeduction==="number"&&<div><span>14,4% — antecipação das parcelas</span><b>- {money.format(preview.calculation.advanceDeduction)}</b></div>}{typeof preview.calculation.contractFee==="number"&&<div><span>10% — multa/taxa do sistema</span><b>- {money.format(preview.calculation.contractFee)}</b></div>}<div><span>Total de descontos previstos</span><b>- {money.format(preview.calculation.deduction)}</b></div><hr/><div className="total"><span>Estorno estimado</span><b>{money.format(preview.calculation.estimatedRefund)}</b></div><small>Prévia calculada com base na regra operacional informada de 14,4% + 10%. O valor final deve ser conferido pelo financeiro antes do pagamento.</small></div>:<div className="info-box">{preview?.reason||"Seu contrato seguirá para conferência da equipe antes da definição de qualquer estorno."}</div>}</>}
      {step===3&&<>{!protocol?<><div className="term-box"><h3>Dados do termo</h3><p>Preencha somente o necessário. O sistema já vincula nome, matrícula, unidade, plano, motivo e data ao pedido. Não é necessário e-mail nem SMTP para concluir.</p></div><label>Endereço<input value={address} maxLength={240} onChange={e=>setAddress(e.target.value)} placeholder="Endereço completo"/></label>{contract&&!contract.recurring&&<label>Chave PIX para eventual estorno<input value={pixKey} maxLength={180} onChange={e=>setPixKey(e.target.value)} placeholder="Somente se houver estorno"/></label>}<label className="check"><input type="checkbox" checked={accepted} onChange={e=>setAccepted(e.target.checked)}/><span>Revisei os dados e confirmo que desejo solicitar o cancelamento.</span></label></>:<div className="download-panel"><h3>Termo pronto para assinatura</h3><b>Protocolo #{protocol}</b><a className="btn primary" href={termUrl}>Baixar termo em PDF</a><span>Assine o documento e depois envie o arquivo assinado na próxima etapa.</span></div>}</>}
